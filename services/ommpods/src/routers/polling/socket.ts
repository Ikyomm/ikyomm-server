...
import {
  broadcastPollingDataForPod,
  getPodPollingSubscriberCount,
  sendPollingData,
  subscribeToPodPolling,
} from "./realtime";
import { readPollingDataFromRedis, refreshPollingDataForPod } from "./state";
...
function startPodTick(podId: string) {
  ...
  const interval = setInterval(async () => {
    if (getPodPollingSubscriberCount(podId) === 0) {
      clearInterval(interval);
      podTickIntervals.delete(podId);
      return;
    }

    const data = await readPollingDataFromRedis(podId);
    if (data) {
...
async function sendInitialPollingData(podId: string, socket: WebSocket) {
  const data = (await readPollingDataFromRedis(podId)) ?? (await refreshPollingDataForPod(podId));
  sendPollingData(socket, data);
}

function acceptSocketConnection(podId: string, socket: WebSocket) {
  const unsubscribe = subscribeToPodPolling(podId, socket);
  startPodTick(podId);
  sendInitialPollingData(podId, socket).catch((error) => {
