...
import {
  ...
  validateMemberLocationAssignment,
} from "./utils";
...
registerOpenApiRoute(companyMembersGroup, create, async (c) => {
  ...
  const locationValidation = await validateMemberLocationAssignment(body);
...
registerOpenApiRoute(companyMembersGroup, update, async (c) => {
  ...
  const locationValidation = await validateMemberLocationAssignment({
    regionId: body.regionId === undefined ? existingMember.regionId : body.regionId,
    zoneId: body.zoneId === undefined ? existingMember.zoneId : body.zoneId,
    locationId: body.locationId === undefined ? existingMember.locationId : body.locationId,
  });
  if (!locationValidation.valid) {
    return c.json(
      createErrorResponse({
